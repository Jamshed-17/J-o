...
5. Создать класс Inventory, добавить методы для основных операций с
инвентарём(добавление, удаление, просмотр сведений об
инвентаре).
...
class Inventory {
    ...
    fun addItem(item: String) {
        items.add(item)
        println("Предмет '$item' добавлен в инвентарь.")
    }

    fun removeItem(item: String) {
        if (items.remove(item)) {
            println("Предмет '$item' удален из инвентаря.")
        } else {
            println("Предмет '$item' не найден в инвентаре.")
        }
    }
